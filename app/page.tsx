...
import { careerDataByLocale } from "@/data/career";
import {
  publicationsDataByLocale,
  getPublicationIcon,
...
const translations = {
  ja: {
    ...
    profileHeadline:
      "大阪公立大学 情報学研究科 修士1年\n株式会社Cmiru エンジニア",
    cmiruProfile: "Cmiru Profile",
    aboutTitle: "About Me",
    interestsTitle: "Interests",
    skillsTitle: "Skills",
    careerTitle: "Career",
    publicationTitle: "Publications & Awards",
    articleTitle: "Articles & Blog",
    projectTitle: "Projects",
    projectStatusOngoing: "進行中",
    projectProblemLabel: "背景課題",
    projectRoleLabel: "あなたの役割",
    projectImplementationLabel: "実装",
    projectResultLabel: "結果",
    projectLearningLabel: "学び",
    publicationLinkFallback: "リンク",
    articleSource: "Cmiru Blog",
    articleDate: "2025.10",
    articleHeading: "GitHub Spec-kitの内部動作から学ぶ、仕様駆動開発（SDD）",
    articleSummary:
  ...
  en: {
    ...
    profileHeadline:
      "M1, Graduate School of Informatics, Osaka Metropolitan University\nEngineer at Cmiru Inc.",
    cmiruProfile: "Cmiru Profile",
    aboutTitle: "About Me",
    interestsTitle: "Interests",
    skillsTitle: "Skills",
    ...
    projectImplementationLabel: "Implementation",
    projectResultLabel: "Result",
    projectLearningLabel: "Learning",
    publicationLinkFallback: "Link",
    articleSource: "Cmiru Blog",
    articleDate: "2025.10",
    articleHeading:
      "Learning Specification-Driven Development (SDD) from the Internals of GitHub Spec-kit",
    articleSummary:
      "A practical discussion on how to run a specification-centered development cycle while leveraging AI tools such as GitHub Copilot.",
  ...
  Locale,
  {
    nav: Array<{ name: string; href: string; icon: typeof User }>;
...
export default function Home() {
  ...
  const careerData = careerDataByLocale[locale];
  const publicationsData = publicationsDataByLocale[locale];
        ...
        <aside className="lg:sticky lg:top-0 lg:h-screen lg:overflow-y-auto px-6 py-12 lg:py-24 border-b lg:border-b-0 lg:border-r border-neutral-200 bg-zinc-50 z-10">
          ...
          >
            <div>
              ...
              <p className="mt-4 text-base text-neutral-600 leading-relaxed font-medium whitespace-pre-line">
                {t.profileHeadline}
              </p>
              <div className="mt-4 inline-flex rounded-md border border-neutral-200 p-1 bg-white">
                <button
                  onClick={() => setLocale("ja")}
                  className={`px-3 py-1.5 text-xs font-semibold rounded ${locale === "ja" ? "bg-neutral-900 text-white" : "text-neutral-600 hover:text-neutral-900"}`}
                >
                  JP
                </button>
                <button
                  onClick={() => setLocale("en")}
                  className={`px-3 py-1.5 text-xs font-semibold rounded ${locale === "en" ? "bg-neutral-900 text-white" : "text-neutral-600 hover:text-neutral-900"}`}
                >
                  EN
                </button>
              </div>
            ...
            <div className="flex flex-col gap-2 text-sm text-neutral-500">
              ...
              <a
                href="https://cmiru.com/member/namisakitakuma"
                target="_blank"
                rel="noreferrer"
                className="flex items-center hover:text-blue-600 transition-colors w-fit"
              >
                <ExternalLink className="w-4 h-4 mr-2" /> {t.cmiruProfile}
              </a>
              <a
                href="https://github.com/nekomanma91823"
                target="_blank"
                rel="noreferrer"
                className="flex items-center hover:text-blue-600 transition-colors w-fit"
              >
        ...
        <main className="min-w-0 px-6 lg:px-12 py-12 lg:py-24 space-y-24">
          ...
          >
            ...
            <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div>
                <h3 className="text-sm font-semibold text-neutral-900 mb-2 uppercase tracking-wider">
                  {t.interestsTitle}
                </h3>
              ...
              {careerData.map((item) => (
                <div key={item.id} className="relative pl-5 sm:pl-8">
                  ...
                  <div className="absolute top-0 right-full pr-4 sm:pr-6 w-24 sm:w-28 text-right">
                    ...
                    {item.current && (
                      <span className="block text-xs font-medium text-blue-500 font-mono mt-0.5">
                        Present
                      </span>
                    )}
                  </div>
                  ...
                  {/* ================= タイムラインの点 (中央) ================= */}
                  <div
                    className={`absolute w-3 h-3 rounded-full -left-[6px] top-1.5 ring-4 ring-zinc-50 ${item.current ? "bg-blue-500" : "bg-neutral-300"}`}
                  />
          ...
          >
            ...
            <div className="space-y-4">
              {publicationsData.map((pub) => {
                ...
                const typeLabel = getPublicationTypeLabel(pub.type, locale);

                return (
                  <Card
                    key={pub.id}
                    className="hover:border-neutral-300 transition-colors shadow-sm"
                  >
                    <CardContent className="pt-6">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-xs text-neutral-500 font-mono">
                          {pub.year}
                          {pub.month
                            ? `.${pub.month.toString().padStart(2, "0")}`
                            : ""}
                        </span>
                      </div>
                      <h3 className="text-lg font-medium text-neutral-900 leading-snug">
                      ...
                      {pub.link && (
                        <div className="mt-4">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center text-sm text-neutral-500 hover:text-blue-600 transition-colors"
                          >
                            <ExternalLink className="w-4 h-4 mr-1" />{" "}
                            {pub.linkText || t.publicationLinkFallback}
                          </a>
                        </div>
                      )}
                    </CardContent>
            ...
            <div className="space-y-4">
              <Card className="hover:border-neutral-300 transition-colors shadow-sm">
                <CardContent className="pt-6">
                  ...
                  <div className="mt-4">
                    <a
                      href="https://cmiru.com/corporate/blog/b85328ba-7223-4fa3-a59a-4e3ca7cbb6fe"
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center text-sm text-neutral-500 hover:text-blue-600 transition-colors"
                    >
                      <ExternalLink className="w-4 h-4 mr-1" /> {t.readArticle}
                    </a>
                  </div>
                </CardContent>
              </Card>
          ...
          >
            ...
            <div className="space-y-5">
              {projectsData.map((project) => (
                ...
                >
                  <CardHeader>
                    <CardTitle className="flex flex-wrap items-center gap-2 text-neutral-900">
                      <span>{project.title}</span>
                      {project.status === "ongoing" && (
                        <Badge variant="secondary">{t.projectStatusOngoing}</Badge>
                      )}
                    </CardTitle>
          ...
          <footer className="pt-24 pb-8 border-t border-neutral-200 text-center text-sm text-neutral-500">
            <p>
              © {new Date().getFullYear()} Takuma Namisaki. All rights reserved.
            </p>
          </footer>
        </main>
      </div>
            ...
            >
              <div className="flex flex-col h-full p-6">
                ...
                <div className="flex justify-center gap-2 pt-6">
                  <button
                    onClick={() => setLocale("ja")}
                    className={`px-3 py-1.5 text-xs font-semibold rounded border ${locale === "ja" ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-300 text-neutral-600"}`}
                  >
                    JP
                  </button>
                  <button
                    onClick={() => setLocale("en")}
                    className={`px-3 py-1.5 text-xs font-semibold rounded border ${locale === "en" ? "bg-neutral-900 text-white border-neutral-900" : "border-neutral-300 text-neutral-600"}`}
                  >
                    EN
                  </button>
                </div>
